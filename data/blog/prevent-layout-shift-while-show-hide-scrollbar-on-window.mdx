...
![thumbnail-image](/static/images/window.jpg)

<UnsplashPhotoInfo
  photoURL="https://unsplash.com/photos/AWSHg2mK-Ho"
...
you should also add the `margin-left` to the `fixed` element.
...
- The first one is to tell your users not to use **Window** device and to use a **Mac** or a **Linux** device instead <Twemoji emoji="grinning-squinting-face" /><Twemoji emoji="grinning-squinting-face" /><Twemoji emoji="grinning-squinting-face" />

- The second one is to just... leave it as it is, sometimes it's not a big deal, and your user doesn't actually care about this layout shift <Twemoji emoji="man-shrugging" />
