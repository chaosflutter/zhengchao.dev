---
title: 'Migrating assets from old shopify store to new store: A step-by-step guide'
date: '2023-02-25'
...
<UnsplashPhotoInfo
  photoURL="https://unsplash.com/photos/WOn90Iui_08"
...
- **Product**: You can download a **CSV** file containing product data and upload it to the new store.
  ...
  <Twemoji emoji="warning" /> With **CSV** files, information about Collections will
  not be migrated, so you will need to recreate the Collection data.
...
- <Twemoji emoji="warning" /> note: The `Content-Type` header must use the value
  `application/json`, not `application/graphql` (Shopify has [note](https://shopify.dev/apps/auth/basic-http#shopify-access-token)
  this)
