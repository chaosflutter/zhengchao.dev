...
date: '2024-04-02'
topics: ['react']
jj: 'https://juejin.cn/post/7353138889458810932'
---
...
但 `shadcn/ui` 不一样，你可以直接通过 cli 工具下载它包含的某个组件到本地目录，然后你可以在本地对组件源码进行修改。它底层依赖的是 [radix](https://www.radix-ui.com/) 组件库，实际也没办法做到百分百可定制性，但确实要比其他组件库灵活太多了。并且，`shadcn/ui` 基于 `tailwind.css` 样式方案，调整组件样式非常方便。组件的默认样式、交互做得都挺精致的。总之，它的开发体验和用户体验做得都不错，也难怪如此热门，建议还没尝试过的同学下一个项目可以考虑用一用 `shadcn/ui`。
