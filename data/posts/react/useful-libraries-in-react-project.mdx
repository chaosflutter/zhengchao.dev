---
title: '盘点最近 React 项目中用到的一些非常好用的库：动画库、组件库、Hook库等'
date: '2024-04-02'
topics: ['react']
---
...
但 `shadcn/ui` 不一样，你可以直接通过 cli 工具下载它包含的某个组件到本地目录，然后你可以在本地对组件源码进行修改。它底层依赖的是 [radix](https://www.radix-ui.com/) 组件库，实际也没办法做到百分百可定制性，但确实要比其他组件库灵活太多了。并且，`shadcn/ui` 基于 `tailwind.css` 样式方案，调整组件样式非常方便。组件的默认样式、交互做得都挺精致的，用户体验很好。总之，它的开发体验和用户体验做得都不错，也难怪如此热门，建议还没尝试过的同学下一个项目可以考虑用一用 `shadcn/ui`。


### lucide-react 图标库

- 官网地址：https://lucide.dev/guide/packages/lucide-react
- 该图标库截至目前已经在 [Github](https://github.com/lucide-icons/lucide) 上收获了 8.3k 的 star
...
`react-string` 上手也很简单，官方提供了很多例子，可以直接看例子的代码来学习和使用，比如这个 [codesandbox 上的例子](https://codesandbox.io/s/exkei)。

**产品有优雅的动画，就像一个人有了灵气，会变得鲜活和灵动，用户体验和感受会好很多。建议还没用过的同学，下一个项目可以试一试。**
