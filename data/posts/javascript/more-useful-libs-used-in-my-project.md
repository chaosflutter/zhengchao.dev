...
之前写过两篇文章，分别介绍了我在 [Siphon](https://siphon.ink) 产品中用到的好用的三方库，以及浏览器插件开发相关的库：
...
今天这篇可能是这个主题的最后一篇，整理了我在第二款产品 [Memowise](https://memowise.ink) 中用到的非常用好的三方库，或许你以后的项目中也能用上这些好用的库。
...
Magic UI 也是一个组件库，但比较特别，它是专为 Landing Page（落地页）开发的组件库。它提供了非常多好用好看甚至有些酷炫的组件，有非常多的 Landing Page 就是基于它开发的，比如官网 [Showcase](https://magicui.design/showcase) 页面下展示的案例：

![](https://blog-1258648987.cos.ap-shanghai.myqcloud.com/blog/more-useful-libs-used-in-my-project/landing_page_showcase.png)

它受 [shadcn/ui](https://ui.shadcn.com) 影响，可以直接通过 shadcn/ui 的 cli 工具下载使用某个组件，下载命令如下所示：
...
我在 [Memowise](https://memowise.ink) 落地页中用到了 [Marquee](https://magicui.design/docs/components/marquee)、[Particles](https://magicui.design/docs/components/particles)、[Border Beam](https://magicui.design/docs/components/border-beam) 等好用的组件。**添加了这些细微精致的动效组件后，整个落地页的气质就不一样了。可能不恰当，但就像一个素颜姑娘化了优雅的淡妆。** 以下的背景使用了 `Particles` 组件，注意背景中可以随着鼠标移动产生视差动效的白色小粒子：
...
在 `LLM` 的时代，这个组件显得特别有用。**比如你可以让大语言模型返回 Markdown 格式的内容，然后用这个组件把 Markdown 文本渲染成适合阅读的网页内容。** 另外，React-Markdown 支持对渲染的内容（h1、p 元素等等）自定义样式。下面图片是 [Memowise](https://memowise.ink) 的 AI Chat 功能，其中 AI 返回的内容就是使用 React-Markdown 渲染的：
...
Slate.js 是众多开源富文本编辑器中非常有特色的一个。我自己在做技术预研的时候，阅读过不少富文本编辑器的使用文档，对 Slate.js 的设计理念最是认同。

**和很多其他富文本编辑器不一样的是，Slate 没有严格的 Schema 约束，且支持更现代化的开发方式，比如所有编辑器元素都可以基于 React 组件实现。** 正如它文档中说的，很多编辑器似乎开箱即用，但要基于这些方案去实现类似 Medium、Google Docs 这样复杂的编辑器几乎是不太可能的。但 Slate 理论上完全可以做到，我自己实际去开发去使用后，也相信它能做到，它的UI、功能，几乎一切都可以定制化。
...
一个挺好用的，完全基于前端实现的摸索搜索库。我在 Memowise 的笔记搜索功能中有使用到它，基本能满足需求，检索速度也很快。但毕竟是纯前端实现的搜索，匹配准确性并不算特别高，尤其是中文场景。它的中文分词算法似乎不是特别有效，很多时候会搜不到。
...
- Github 地址：[https://github.com/vercel/react-tweet](https://github.com/vercel/react-tweet)
- 该库截至目前已经在 Github 上收获了 1.6k 的 star

React-tweet 是一个非常好用的库，可以基于 tweet id 渲染出和 twitter 官方样式非常接近的 tweet。如果你的应用涉及到 twitter 的推文展示，那么大概率可以使用这个组件来改善推文的阅读体验。
